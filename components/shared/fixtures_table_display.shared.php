...
include_once __DIR__ . "/../shared/renders/display-league-headers.shared.php";

function FixtureDetails($data): void {
    // Group fixtures by league_id without sorting explicitly
    $groupedData = [];
    
    foreach ($data as $fixture) {
        $leagueId = $fixture['game_details']['league_id'];
    ...
    foreach ($groupedData as $leagueId => $fixtures) {
        ...
        foreach ($fixtures as $fixture_details) {
            ...
            if (is_array($fixture_details) && isset($fixture_details['game_details'])) {
                ...
                $myNewDateString = explode(' ', $myNewDateString)[0]; // Get only the date part
            }
            ...
            $url_name = str_replace('+', '-', urlencode(strtolower(trim($fixture_details['game_details']['home_team_name'] . '-vs-' . $fixture_details['game_details']['away_team_name'] . '-' . $fixture_details['game_details']['fixture_id']))));
