...
function DisplayStandings($league_id,$homeTeamId, $awayTeamId, $deviceType) {
    $fetched_standings = fetchTableStandings($league_id);

    if($fetched_standings["message"] === "success"){
        echo displayIndependentLeagueStandings($fetched_standings["data"],$homeTeamId, $awayTeamId, $deviceType);
    }
}
...
function displayIndependentLeagueStandings($standingsData, $homeTeamId, $awayTeamId, $deviceType) {
    ...
    if (!empty($standings) && is_array($standings[0])) {
        ...
        foreach ($standings[0] as $teamData) {   
            ...
            $highlight = (($homeTeamId == $teamData['team']['id']) || ($awayTeamId == $teamData['team']['id'])) ? 'background-color: #FAEBD7;' : 'cursor: auto;';
            ...
            // Team standings row
            $standingsTableList .= '
                <div class="responsive-row" style="' . $highlight . '">
